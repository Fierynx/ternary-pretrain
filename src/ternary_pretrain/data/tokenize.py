    ...
    for split in ("train", "validation"):
        ...
        with source_path.open("r", encoding="utf-8") as stream:
            for line in stream:
                ...
                if (
                    shard_token_ids
                    and len(shard_token_ids) + len(document_token_ids) > config.shard_token_limit
                ):
                    shard_manifests.append(
                        _write_shard(
                            config.token_output_dir,
                            split,
                            len(shard_manifests),
                            shard_token_ids,
        ...
        manifest_path = config.token_output_dir / f"{split}.manifest.json"
